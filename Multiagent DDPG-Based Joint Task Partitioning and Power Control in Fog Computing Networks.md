...
#introduction
隨著運算密集和延遲敏感型應用（例如線上應用）的湧現，在 3D 遊戲、視訊監控和增強/虛擬實境[1]–[3]等領域，行動裝置(MD)不堪重負，難以滿足嚴苛的運算任務需求。將任務卸載到擁有充足運算資源的集中式遠端雲端伺服器是一種可行的方法，但這會導致由於前傳和回傳網路的長距離瓶頸效應而造成嚴重的服務延遲。近年來，霧運算已成為一種流行的範式，它利用不同霧設備(FD)（例如相鄰基地台(BS)、存取點(AP)、行動邊緣運算(MEC)伺服器和終端設備（例如車輛、筆記型電腦））的通訊和運算資源[4]。然而，由於計算資源的限制以及霧設備的異構性[7]，傳統的二進制卸載[5]（將整個任務卸載到單一霧設備）可能會導致較高的任務失敗率。因此，部分卸載是一種更有效率利用資源和能源的方法[6]。在部分卸載中，MD 可以將計算任務分割成多個子任務，並將子任務卸載到不同的 FD 進行並行執行
...
#貢獻
本文研究了具有多個 MD 和 FD 的霧運算網路中的聯合任務分割和功耗控制問題。此聯合任務劃分和功耗控制問題被視為一個==連續決策問題==，旨在最大化系統的長期效用，同時考慮每個 MD 的任務執行延遲和能耗。

考慮到==環境的動態性以及任務分割和功率控制的高維度動作空間==，我們將問題轉換為多用戶隨機卸載博弈，並基於 MADDPG 模型求解問題。在 MADDPG 模型中，每個 MD（多用戶決策者）被建模為一個智能體，僅向集中式評論家網路共享局部資訊和環境觀測數據，同時透過分散式行動者網路進行局部決策。
